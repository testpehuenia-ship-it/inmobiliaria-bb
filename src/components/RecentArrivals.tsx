...
export default function RecentArrivals() {
  ...
  // Track which card is flipped (for mobile touch toggle)
  ...
  const handleCardTouch = useCallback((id: string) => {
      ...
      <div className="carousel-viewport">
        <div className="carousel-track">
          {displayProps.map((property, index) => {
            const cardKey = `${property.id}-${index}`;
            const isFlipped = flippedId === cardKey;
            return (
              <div
                key={cardKey}
                className="flip-card-container"
                onClick={() => handleCardTouch(cardKey)}
              >
